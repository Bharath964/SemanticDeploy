...
HISTORY_PATH = "models/history_custom_unet.json"
PATCH_SIZE = 256
N_CLASSES = 6
...
# New post-processing function for masks
...
# Modified confidence display functions
def plot_confidence(smooth_confidence):
    plt.figure(figsize=(5, 5))
    plt.imshow(smooth_confidence * 100, cmap='viridis', vmin=0, vmax=100)
    plt.colorbar(label='Confidence (%)')
    plt.title("Smooth Tiling Confidence Map")
    plt.axis('off')
    return plt

# Modified prediction section in main app
def main():
    ...
    if st.button("Run Prediction"):
        ...
        with st.spinner("Running smooth tiling prediction..."):
            ...
            img_scaled = scaler.fit_transform(img_rgb.reshape(-1, 3)).reshape(img_rgb.shape)
            smooth_probs = predict_img_with_smooth_windowing(
                img_scaled,
                window_size=PATCH_SIZE,
                subdivisions=2,
                nb_classes=N_CLASSES,
            ...
            smooth_pred = np.argmax(smooth_probs, axis=-1)
            smooth_rgb = label_to_rgb(smooth_pred)
            smooth_confidence = np.max(smooth_probs, axis=-1)
            avg_smooth_conf = np.mean(smooth_confidence) * 100  # Convert to percentage
            
        st.session_state.update({
            'smooth_rgb': label_to_rgb(smooth_pred),
            'smooth_confidence': smooth_confidence,
            'avg_smooth_conf': avg_smooth_conf,
            'patch_pred': patch_pred,
            'patch_rgb': patch_rgb,
            'smooth_pred': smooth_pred,
            'smooth_rgb': smooth_rgb,
            'prediction_done': True
            # ... [rest of session state updates]
        })

    # Modified confidence display
    if st.session_state.get('prediction_done', False):
        ...
        st.subheader("Confidence Map (Smooth Tiling)")
        st.pyplot(plot_confidence(st.session_state['smooth_confidence']))
        st.caption(f"Average Confidence: {st.session_state['avg_smooth_conf']:.1f}%")

        analysis_mode = st.radio("Select Analysis Mode:", 
                            ["Binary Mask Analysis", "Interactive Area Selection"])

    # Modified binary mask section
    if analysis_mode == "Binary Mask Analysis":
        ...
        if st.button("Generate Binary Masks and Compute Total Area"):
            for label_id in selected_ids:
                mask = generate_binary_mask(smooth_pred, label_id)
                mask = postprocess_mask(mask, label_id)  # Add post-processing
